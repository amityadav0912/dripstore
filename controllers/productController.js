...
exports.GetSingleProduct = BigPromise(async (req, res, next) => {
    ...
    if(!product){
        return next(new CustomError(`No product found with this id ${req.params.id}`, 401 ))
    }
...
exports.adminUpdateOneProduct = BigPromise(async (req, res, next) => {
    ...
    if(req.files){
        ...
        const oldImageArray = product.photos;

        for (let index = 0; index < oldImageArray.length; index++) {
            await cloudinary.v2.uploader.destroy(oldImageArray[index].id); 
        }
